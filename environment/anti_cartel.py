...
class AntiCartelMechanism:
    ...
    def _initialize_detection_mechanism(self) -> None:
        ...
        self.sustained_pattern_threshold = int(
            self.monitoring_window * 0.3
        )  # Need 30% of window for pattern

        # Log detection parameters
        print("Detection mechanism initialized with:")
    ...
    def _calculate_ceiling_penalties(
        self, selling_prices: list[float], grid_price: float
    ) -> list[float]:
        """
    ...
    def get_mechanism_status(self) -> dict[str, Any]:
        ...
        """
        status = {"type": self.mechanism_type, "active": self.is_active()}
        ...
        if self.is_active():
            if self.mechanism_type == "detection":
                ...
                status.update(
                    {"markup_limit": self.markup_limit, "market_elasticity": self.market_elasticity}
                )
