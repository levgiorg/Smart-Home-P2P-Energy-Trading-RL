...
class Environment:
    ...
    def _initialize_house_states(self) -> None:
        ...
        self.batteries = [
            round(random.uniform(battery_min + 1, battery_max), 1) for _ in range(self.num_houses)
        ]
    ...
    def _recalculate_dimensions(self) -> None:
        ...
        print(
            f"State dimensions recalculated: {self.STATE_DIM_PER_HOUSE} per house, {self._state_dim} total"
        )
    ...
    def step(
        self, actions: torch.Tensor, thermal_mass_coeff: float = 0.14 * 1000 / ((1 - 32) * 5 / 9)
    ) -> tuple[list[float], list[float], bool, dict[str, Any]]:
        """
        Execute one time step within the environment.

        Args:
            actions: Tensor of shape [num_houses, 3] containing [hvac_energy, battery_action, selling_price] for each house
            thermal_mass_coeff: HVAC system thermal mass coefficient (default derived from physical constants)
        ...
        rewards = self._update_houses_and_calculate_rewards(
            actions, house_energy_status, transactions, infos, thermal_mass_coeff
        )
    ...
    def _execute_p2p_trading(
        self, house_energy_status: list[dict[str, Any]]
            ...
            for seller in sellers:
                ...
                if selling_price < self.price:
                    ...
                    transactions[buyer_id].append(
                        {"seller_id": seller_id, "amount": energy_traded, "cost": transaction_cost}
                    )
    ...
    def _update_houses_and_calculate_rewards(
        ...
        Args:
            ...
            thermal_mass_coeff: HVAC system thermal mass coefficient
        ...
        for i in range(self.num_houses):
            ...
            self._update_temperature(i, hvac_energy, thermal_mass_coeff)
            ...
            energy_metrics = self._calculate_energy_metrics(
                i, house_energy_status, transactions, actions
            )
    ...
    def _update_temperature(
        self, house_idx: int, hvac_energy: float, thermal_mass_coeff: float
    ) -> None:
        """
        ...
        self.inside_temperatures[house_idx] = weight * self.inside_temperatures[house_idx] + (
            1 - weight
        ) * (self.ambient_temperature - (efficiency / thermal_mass_coeff) * hvac_energy)
    ...
    def _update_battery_state(self, house_idx: int, battery_action: float) -> None:
        ...
        if battery_action > 0:
            ...
            self.batteries[house_idx] = min(
                self.batteries[house_idx]
                + self.config.get("environment", "battery_charging_efficiency") * battery_action,
            ...
            self.batteries[house_idx] = max(
                self.batteries[house_idx]
                + battery_action / self.config.get("environment", "battery_discharging_efficiency"),
    ...
    def _calculate_energy_metrics(
        ...
        # Calculate costs
        hvac_energy_cost = 1e-3 * self.price * energy_from_grid + sum(
            t["cost"] for t in transactions[house_idx]
        )

        # Battery depreciation cost
        battery_action = actions[house_idx, 1].item()
        battery_depreciation = self.config.get("cost_model", "depreciation_coeff") * abs(
            battery_action
        )
    ...
    def _apply_anti_cartel_mechanism(
        self, rewards: list[float], infos: dict[str, Any]
    ) -> list[float]:
        """
    ...
    def _load_house_specific_data(self, idx_start: int, idx_end: int) -> None:
        ...
        for data in self.agent_data:
            ...
            self.sun_power_predictions.append(
                gen_df["Predicted Generation"].values[idx_start + 1 : idx_end]
            )
            self.power_demands.append(cons_df["Actual Consumption"].values[idx_start : idx_end - 1])
            self.power_demand_predictions.append(
                cons_df["Predicted Consumption"].values[idx_start + 1 : idx_end]
            )
