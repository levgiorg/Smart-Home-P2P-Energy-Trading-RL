...
class Environment:   
    ...
    def step(self, actions, A=0.14 * 1000 / ((1 - 32) * 5 / 9)):
        ...
        Args:
            actions: Tensor of shape [num_houses, 2] containing [e_t, a_batt] for each house
        """
        # Unscale actions
        actions = self.utilities.unscaler(actions)

        rewards = []
        infos = {'HVAC_energy_cons': [], 'depreciation': [], 'penalty': []}
        ...
        for i in range(self.num_houses):
            ...
            # Power balance
            power_utility_grid = self.power_demand[i] + 1e-3 * e_t - self.sun_power[i] + a_batt

            # Calculate penalties
            hvac_energy_cons = self.price * max(0, power_utility_grid)
            ...
            # Calculate reward
            reward = -self.beta * (hvac_energy_cons + depreciation) - temp_penalty
