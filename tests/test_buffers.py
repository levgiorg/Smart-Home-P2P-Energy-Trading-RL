...
def _push_n(buf, n):
    for _ in range(n):
        buf.push(torch.randn(S), torch.randn(A), torch.randn(S), torch.tensor([0.5]), False)
...
def test_rollout_buffer_gae():
    ...
    for _ in range(64):
        buf.push(
            torch.randn(S), torch.randn(A), torch.tensor([-0.5]), 1.0, torch.tensor([0.3]), False
        )
    assert buf.ready()
...
def test_rollout_buffer_clear():
    ...
    for _ in range(16):
        buf.push(
            torch.randn(S), torch.randn(A), torch.tensor([0.0]), 0.0, torch.tensor([0.0]), False
        )
    buf.clear()
