...
class DoubleDQNNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.q_network_1 = DQNNetwork(
            state_dim, num_actions, fc1_dims, fc2_dims, fc3_dims, dueling=True, device=device
        )
...
class PrioritizedReplayBuffer:
    ...
    def _initialize_buffers(self, state_dim: int):
        ...
        print(
            f"Initializing GPU prioritized replay buffer: {self.capacity} transitions on {self.device}"
        )
    ...
    def add(self, state, action, reward, next_state, done, td_error: float | None = None):
    ...
    def get_memory_usage(self) -> dict:
        ...
        total_elements = self.capacity * (
            self.states.shape[1] * 2 + 3
        )  # 2*state_dim + 3 other fields
        memory_mb = (total_elements * 4) / (1024 * 1024)  # Approximate 4 bytes per element
