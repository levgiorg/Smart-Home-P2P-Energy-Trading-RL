...
class AntiCartelAnalyzer:
    def __init__(self, base_path="ml-outputs"):
        ...
        self.runs = sorted(
            [str(self.base_path / d) for d in os.listdir(self.base_path) if d.startswith("run")]
        )
        ...
        for run_path in group_runs:
            ...
            try:
                ...
                if all(key in data for key in ["selling_prices", "grid_prices"]):
                    price_ratios = (
                        np.array(data["selling_prices"]) / np.array(data["grid_prices"])[:, None]
                    )
    ...
    def compare_mechanisms(self):
        ...
        for name, data in mechanism_data.items():
            if "rewards" in data and len(data["rewards"]) > 0:
                ...
                ax.plot(episodes, ma_rewards, label=labels[name], color=colors[name], linewidth=2)
                ax.fill_between(
                    episodes,
