...
def analyze_prices(run_path):
    ...
    if isinstance(prices, list) and len(prices) > 0:
        ...
        last_100_std = np.mean(std_per_episode[-100:]) if len(std_per_episode) >= 100 else avg_std_all
        ...
        threshold = 0.05
        print(f"Prices are {'similar' if last_100_std < threshold else 'diverse'} in last 100 episodes (threshold {threshold})")
    else:
