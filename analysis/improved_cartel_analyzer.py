...
class ImprovedAntiCartelAnalyzer:
    def __init__(self, base_path="ml-outputs"):
        ...
        self.runs = sorted(
            [str(self.base_path / d) for d in os.listdir(base_path) if d.startswith("run")]
        )
    ...
    def _group_runs(self) -> dict:
        ...
        for run_path in self.runs:
            try:
                ...
                with open(Path(run_path) / "hyperparameters.json") as f:
    ...
    def _calculate_convergence(
        self, rewards: np.ndarray, window: int = 100, threshold: float = 0.05
    ) -> int:
    ...
    def plot_experiment_comparison(self, exp_type: str):
        ...
        ax.bar(conv_df["mechanism"], conv_df["episodes"], yerr=conv_df["std"], capsize=5)
