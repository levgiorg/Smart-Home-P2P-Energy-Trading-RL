...
class RunAnalyzer:
    ...
    def load_run_data(self, run_id):
        ...
        with open(run_path / "hyperparameters.json") as f:
    ...
    def _calculate_convergence(self, rewards, window=100, threshold=0.05):
        ...
        convergence_point = np.where(
            rolling_std.mean(axis=1) / np.abs(rolling_mean.mean(axis=1)) < threshold
        )[0]
    ...
    def analyze_all_runs(self):
        ...
        df_norm = df.select_dtypes(include=[np.number]).apply(
